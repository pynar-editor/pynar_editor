...
import json
import logging
from Components.ChatBotView.Dialog import LoadMessage
from PyQt5.QtCore import Qt
from PyQt5.QtSql import QSqlDatabase
from PyQt5.QtCore import QDir, QFile, Qt
...
class UcChatBotView(QWidget):
    ...
    def createUserButton(self, buttonTextList, errorButtons):
        ...
        for buttonText in buttonTextList:
            if len(buttonText) > lineLetterCapacity or activeLineButtonCount > 3:
                lineLetterCapacity = 37
    ...
    def errorButtonClicked(self, buttonText):
        ...
        self.showUserMessage(message)
        self.textEdit_message.append("\n")
        if buttonText.lower() == 'Evet'.lower():
            self.ShowRunErrorMessage()
        else:
            self.textEdit_message.append(self.robotBalloonMessage(message='Görüşmek Üzere :)'))
    ...
    def RunErrorMessage(self, runErrors):
        if(len(runErrors)>0):
    ...
    def ShowRunErrorMessage(self):
        ...
        line = self.runErrors[0]['range']['start']['line']
        errorLine = self.parent.textPad.text().split("\n")[line].replace("\n", "")
        kNNList = self.kNN(errorLine)
        self.runErrors.pop(0)

        if len(self.runErrors) > 0:
            message = '{0}. Hata <br /><br />{1}.<br/><p style="color:red">Bu hatayı düzeltmek için doğru yazılmış şu örnekleri inceleyiniz</p>'.format(self.errorNo, descErrorMessage)
            for i in kNNList:
                message += i + '<br/>'
    ...
    def kNN(self,errorLine):
        for i in range(len(self.dataSetFile)):
            with open(self.parentDir + "/Data/CodeDataset/"+self.dataSetFile[i],"r") as f:
                if i == 0:
                    data = f.readlines()
                else:
                    data += f.readlines()
        distance = []
        for i in data:
            distance.append(textdistance.strcmp95.distance(i,errorLine))
        _,data= zip(*sorted(zip(distance,data), reverse=False))

        kNNlist = []
        for i in range(5):
            kNNlist.append(data[i].replace("\n","").strip())
        return set(kNNlist)
