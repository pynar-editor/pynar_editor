...
from PyQt5.QtWidgets import QAbstractItemView, QLayout
...
class UcLMMenus(QWidget):
    ...
    def hideTreeView(self, control=False):
        if not self.treeView.isVisible() or control:
            ...
            self.solmenu.setFixedWidth(407)
            return False
        else:
            self.treeView.show()
    ...
    def expanded(self, index):
        dosyaAdi = index.data()  # type: object
    ...
    def AddChild(self, children, parent):
        if isinstance(children, dict):
            for item, items in children.items():
                ...
                if parent.text().split(' ')[0] in (self.descriptions.keys()):
                    parent.setIcon(QIcon(':/icon/images/treeview-help.png'))
                if item.split(' ')[0] in (self.descriptions.keys()):
                    child.setIcon(QIcon(':/icon/images/treeview-help.png'))
    ...
    def setupUi(self, Form):
        ...
        self.treeView.setMinimumSize(QtCore.QSize(290, 410))
        self.treeView.setMaximumSize(QtCore.QSize(290, 16777215))
    ...
    def FillHelpDialogWords(self): #Help Dialog için Treeview öğeleri ile doldurulur.
        for dosya in self.jsonFiles:
            ...
            with open(dosyaPath, encoding='utf-8') as f:
                data = json.load(f)
            for root, children in data.items():
                if root[:root.find("#")] in self.pathDescriptions:
